...
Team Engine v2: Production Backtesting Engine

SIGNAL DISCOVERY:
    Hidden alpha = TICKER_01 lag-1 return predicts TICKER_00 return.
    Directional accuracy: ~57.1% long / ~58.0% short
    Spearman IC = 0.190 at lag-1 (dominant predictor; 2nd place TICKER_25 IC=0.010).

FOUR-TICKER STRUCTURE (organizer hint):
    TICKER_00 = target traded instrument
    TICKER_01 = primary causal predictor (IC 19× stronger than any other)
    TICKER_25 = secondary condition ticker (appears in all two-ticker interactions)
    4th ticker = a contemporaneous correlate of TICKER_00; identified in hint but not
                 independently traded — acts as confirmation context only.
    WHY WE DO NOT ADD TICKER_25 AS A HARD FILTER:
        At the signal frequencies needed for fee amortisation (≥4 k bars), the
        two-ticker (T01 ∩ T25) hit rate is 0.563–0.573, i.e. ≤ single-T01 at 0.571.
        The only superior two-ticker result (hit_rate 0.595) uses only 195 signals
        over 10 years — too sparse to produce a competitive Sharpe after 10 bps fees.
        TICKER_25's own IC (0.010) is 19× weaker than TICKER_01 and adds no marginal
        predictive power at useful signal frequencies.

STRATEGIES:
    Core signal driver: EMA-based trend-following on TICKER_01 returns.

    Long-Only (unchanged from v1):
        Entry : Fast EMA (span=25) of TICKER_01 returns crosses above 0 → BUY TICKER_00
        Exit  : Slow EMA (span=2000) crosses below −0.0001 → CLOSE
        Params: cap $1,000,000, notional $850,000

    Long-Short (FIXED from v1):
        v1 bug : LS_EXIT_SPAN=1500, LS_EXIT_THRESH=-0.001 → slow EMA NEVER reaches
                 this threshold over 10 years → only 26 trade bars → pure buy-and-hold
                 masquerading as long-short → Sharpe 0.42 (below buy-and-hold quality)
        v2 fix : Use SAME EMA spans and exit threshold as Long-Only (span=25/2000,
                 thresh=-0.0001) but with long_only=False so the SHORT leg actually fires.
        Entry LONG  : Fast EMA > 0  → BUY  $1,800,000 TICKER_00
        Entry SHORT : Fast EMA < 0  → SELL $400,000 TICKER_00
        Exit LONG   : Slow EMA < −0.0001 → CLOSE
        Exit SHORT  : Slow EMA > +0.0001 → CLOSE
        Discovery support: T01 negative returns predict T00 negative with hit_rate 0.58
                           (conditional_rules.csv line 2; volume_state_results top shorts).
...
# ── Long-Only parameters (unchanged from v1) ────────────────────────────────
...
# ── Long-Short parameters (FIXED: now use same EMA structure as LO) ─────────
# v1 had LS_ENTRY_SPAN=20, LS_EXIT_SPAN=1500, LS_EXIT_THRESH=-0.001
# EMA(1500) of 5-min returns never reached -0.001 → only 26 trade bars in 10 years
# → strategy was effectively buy-and-hold (never exited, never went short)
LS_ENTRY_SPAN    = 25            # FIXED: was 20 → now same as LO for consistency
LS_EXIT_SPAN     = 2000          # FIXED: was 1500 → now same as LO so exits actually fire
LS_EXIT_THRESH   = -0.0001       # FIXED: was -0.001 → now same as LO so signal cycles
LS_LONG_NOTIONAL = 1_800_000.0   # long position notional (unchanged)
LS_SHORT_NOTIONAL = 400_000.0    # short position notional (unchanged, conservative)
LS_CAP_TARGET    = 1_980_000.0   # trim target (unchanged)
...
def main():
    ...
    t0 = time.time()
    print(f"\n{'='*60}")
    print(f"  MOCCM 2026 Signal Hunt — Team: {team_name.upper()}  [engine v2]")
    print(f"{'='*60}")
